...
def memoryUnitToG(str):
    if str.endswith("M") or  str.endswith("Mi") or str.endswith("m"):
...
def node_resource(core_api, n_name=None):
    ...
    for node in core_api.list_node().items:
        ...
        node_resouces[node_name] = {"allocatable_cpu": "", "capacity_cpu":"", "allocatable_memory": "","capacity_memory":"",
      ...
      for c in pod.spec.containers:
          ...
          if c.resources.requests is not None:
              ...
              if  "memory" in c.resources.requests:
                  ...
                  node_resouces[node_name]['memory_requests'] = round(node_resouces[node_name]['memory_requests'] + memoryUnitToG(memory_request),2)
          # 资源限制
